...
class ThemesBackup
{
    ...
    public function xuses($theme)
    {
        ...
        if($theme_type == null) {
            throw new UnknownThemeException("Theme [$theme] type not set.");
        }

        $this->type = $theme_type;
    ...
    {
        $themes = $this->manager->all()->toArray();
    ...
    /**
     * Gets current theme type.
     *
     * @return string
     */
    public function getType()
    {
        return $this->type ?: $this->getConfig('type');
    }
    ...
    protected function getModuleView($view)
    {
        if (app('Veemo\Core\Modules\Modules')) {
            ...
            if ($viewSegments[0] == 'modules') {
                ...
                return "module.{$module}::{$type}.{$view}";
            }
